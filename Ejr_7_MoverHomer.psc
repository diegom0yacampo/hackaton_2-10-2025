...
Algoritmo STAR_WARS
	...
	Mientras salir = Falso //Para que el juego se ejecute hasta que el usuario quiera salir
		Para i = 1 Hasta largo Hacer //Pasamos por todos los valores del matriz por toda su longitud y le damos un numero aleatorio
			...
			Escribir "";
		FinPara
		//Menu del juego con opciones a elegir
		Escribir "¿Quieres mover a homer?"
		Escribir "(1) Mover a homer a la derecha";
		Escribir "(2) Mover a homer a la izquierda";
		Escribir "(3) Salir del programa";
		Leer user_input; //Leemos la eleccion del usuario
		Si user_input == "1"
			Si ancho_homer < largo
		...
		Si user_input == "2"
			Si ancho_homer > 1
				...
				Escribir "JOB DONE"
			SiNo
				Escribir "Homer ya esta en el borde IZQUIERDO"
			FinSi
		FinSi
		Si user_input == "3"
			salir = Verdadero
		...
		FinSi
		Si user_input <> "1" y user_input <> "2" y user_input <> "3" //Mensaje error
